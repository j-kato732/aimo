...
type AimModelORM struct {
	...
	AchievementMeans            []*AchievementMeanModelORM `gorm:"foreignkey:AimModelId;association_foreignkey:Id"`
...
func (m *AimModel) ToORM(ctx context.Context) (AimModelORM, error) {
	...
	for _, v := range m.AchievementMeans {
		if v != nil {
			if tempAchievementMeans, cErr := v.ToORM(ctx); cErr == nil {
				to.AchievementMeans = append(to.AchievementMeans, &tempAchievementMeans)
			} else {
				return to, cErr
			}
		} else {
			to.AchievementMeans = append(to.AchievementMeans, nil)
		}
	}
...
func (m *AimModelORM) ToPB(ctx context.Context) (AimModel, error) {
	...
	for _, v := range m.AchievementMeans {
		if v != nil {
			if tempAchievementMeans, cErr := v.ToPB(ctx); cErr == nil {
				to.AchievementMeans = append(to.AchievementMeans, &tempAchievementMeans)
			} else {
				return to, cErr
			}
...
func DefaultStrictUpdateAimModel(ctx context.Context, in *AimModel, db *gorm.DB) (*AimModel, error) {
	...
	filterAchievementMeans := AchievementMeanModelORM{}
	if ormObj.Id == 0 {
		return nil, errors.EmptyIdError
	}
	filterAchievementMeans.AimModelId = new(int64)
	*filterAchievementMeans.AimModelId = ormObj.Id
	if err = db.Where(filterAchievementMeans).Delete(AchievementMeanModelORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultApplyFieldMaskAimModel(ctx context.Context, patchee *AimModel, patcher *AimModel, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*AimModel, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"AchievementMeans" {
			patchee.AchievementMeans = patcher.AchievementMeans
			continue
		}
...
func DefaultApplyFieldMaskAchievementMeanModel(ctx context.Context, patchee *AchievementMeanModel, patcher *AchievementMeanModel, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*AchievementMeanModel, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"MeanNumber" {
			patchee.MeanNumber = patcher.MeanNumber
			continue
		}
		if !updatedCreatedAt && strings.HasPrefix(f, prefix+"CreatedAt.") {
			if patcher.CreatedAt == nil {
